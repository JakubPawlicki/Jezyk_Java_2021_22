...
public class DataFrame {
    ...
    public void ilocSet(int i, String c, String value) {
        ...
        throw new IllegalArgumentException("Column '" + c + "' not in pl.edu.fais.uj.dfvalue.DataFrame");
    }
        ...
        for (String columnsToAddNames : cols) {
            ...
            for (String colname : colnames) {
                ...
                if (columnsToAddNames.equals(colname)) {
                    ...
                    if (from >= 0) {
                        if(to < from)
                            throw new IllegalArgumentException("To should be longer than from");
    ...
    public DataFrame addRow(String[] datarow, boolean inplace) {
        if(datarow.length != this.coltypes.size())
            throw new IllegalArgumentException("Length of datarow and existing columns should be equal");
