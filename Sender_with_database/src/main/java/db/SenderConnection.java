...
public class SenderConnection {
    ...
    public <T> Optional<T> executeQuery(String statement, SenderMapper<T> mapper) throws DatabaseException {
        if (connection != null) {
            try {
                ...
                ResultSet rs = statement1.executeQuery(statement);
                return mapper.get(rs);
            } catch (SQLException ex) {
                throw new DatabaseException("Can't create a statement object %s".formatted(url), ex);
    ...
    /**
     * returnGeneratedKeys: true: returns id of inserted record, false: returns 1 if record updated
     **/
        ...
        if (connection != null) {
            try {
                try (Statement cStatement = connection.createStatement()) {
                    // TODO read: https://stackoverflow.com/questions/24378270/how-do-you-determine-if-an-insert-or-update-was-successful-using-java-and-mysql
                    long updated = cStatement.executeUpdate(statement, Statement.RETURN_GENERATED_KEYS);
                    if (returnGeneratedKeys) {
                        // TODO read: https://stackoverflow.com/questions/4246646/mysql-java-get-id-of-the-last-inserted-value-jdbc
